...
`;var st=3e4,nt=1e3,V=class extends m{constructor(){super();this._hasProgress=!1;this._versionChecked=!1;this._busy=new Set,this._expanded=new Set}static getStubConfig(){return{}}static getConfigForm(){return{schema:[{name:"title",selector:{text:{}}}]}}setConfig(t){this._config=t}getCardSize(){if(!this.hass)return 2;let t=M(this.hass),s=pe(t.filter(L).sort(le)).reduce((l,a)=>l+3+(a.superseded.length?1+(this._expanded.has(a.alert.entityId)?a.superseded.length*3:0):0),0),r=t.filter(l=>l.state==="no_data").length,o=t.some(l=>l.state==="disabled");return 1+Math.max(1,s)+(r?1+r:0)+(o?1:0)}getGridOptions(){return{columns:12,min_columns:6}}connectedCallback(){super.connectedCallback(),this._tick=window.setInterval(()=>this.requestUpdate(),st)}disconnectedCallback(){super.disconnectedCallback(),window.clearInterval(this._tick),window.clearTimeout(this._progressTick),this._progressTick=void 0}shouldUpdate(t){if(t.size!==1||!t.has("hass"))return!0;let i=t.get("hass");if(!i||!this.hass||i.themes?.darkMode!==this.hass.themes?.darkMode)return!0;let s=this.hass.states,r=i.states;for(let o in s)if(S(o)&&s[o]!==r[o])return!0;for(let o in r)if(S(o)&&!(o in s))return!0;return!1}updated(){this._hasProgress&&this._progressTick===void 0&&this.isConnected&&(this._progressTick=window.setTimeout(()=>{this._progressTick=void 0,this.requestUpdate()},nt)),this.hass&&!this._versionChecked&&(this._versionChecked=!0,this._checkVersion())}async _checkVersion(){try{let{version:t}=await this.hass.callWS({type:"alert_redux/info"});t!=="0.11.0"&&(this._serverVersion=t)}catch{}}render(){if(this._hasProgress=!1,!this.hass||!this._config)return c;let t=M(this.hass),i=t.filter(L).sort(le),s=t.filter(a=>a.state==="no_data").sort(Pe),r=t.filter(a=>a.state==="disabled").length,o=this._config.title,l=this.hass.themes?.darkMode??!1;return d`
            ...
            <div class="meta">
              ...
              ${r?d`<span>·</span>
                    <span title=${r.toLocaleString(s)}
                      >firing for ${B(r)} (since ${b(r,s)})</span
                    >`:c}
    ...
    `}_kind(e){let t=this.hass?.states[e.entityId],i=t?this.hass?.formatEntityAttributeValue?.(t,"kind"):void 0;return i&&i!==e.kind?i:Te[e.kind]??e.kind}_generated(e){if(!e.generatedBy)return c;let i=this.hass?.states[e.generatedBy]?.attributes.friendly_name??e.generatedBy;return d`, <span class="generated" title=${`Generated by ${i}`}>generated</span>`}_state(e){let t=this.hass?.states[e.entityId],i=t?this.hass?.formatEntityState?.(t):void 0;return i&&i!==e.state?i:Re[e.state]??e.state}_detail(e){let t=this.hass?.locale?.language;return e.state==="disabled"?e.disabledUntil?d`until ${b(e.disabledUntil,t)}`:c:e.state==="ack"&&e.snoozedUntil?d`snoozed until ${b(e.snoozedUntil,t)}`:L(e)&&e.firingSince?d`since ${b(e.firingSince,t)}`:e.state==="no_data"&&e.noDataSince?d`for ${B(e.noDataSince)}`:c}_defaultUntil(){let e=new Date;e.setDate(e.getDate()+1),e.setHours(8,0,0,0);let t=i=>String(i).padStart(2,"0");return`${e.getFullYear()}-${t(e.getMonth()+1)}-${t(e.getDate())}T${t(e.getHours())}:${t(e.getMinutes())}`}_toggleMenu(e){this._untilOpen=!1,this._menu=this._menu===e.entityId?void 0:e.entityId}_suspendUntil(e){let t=this.renderRoot.querySelector('input[type="datetime-local"]');if(!t?.value)return;let i=new Date(t.value);Number.isNaN(i.getTime())||this._suspend(e,{until:i.toISOString()})}_suspend(e,t){this._menu=void 0,this._untilOpen=!1,this._call(e,"suspend",t)}async _call(e,t,i={}){if(this.hass){this._busy=new Set(this._busy).add(e.entityId);try{await this.hass.callService("alert_redux",t,{entity_id:e.entityId,...i})}catch(s){this._fire("hass-notification",{message:s?.message??String(s)})}finally{let s=new Set(this._busy);s.delete(e.entityId),this._busy=s}}}_moreInfo(e){this._fire("hass-more-info",{entityId:e.entityId})}_fire(e,t){this.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0}))}};j.properties={hass:{attribute:!1},_config:{state:!0},_busy:{state:!0},_menu:{state:!0},_untilOpen:{state:!0}},j.styles=[J,k`
      .content {
        display: flex;
        flex-direction: column;
        gap: 8px;
    ...
    `];customElements.get("alert-redux-admin-card")||(customElements.define("alert-redux-admin-card",j),window.customCards=window.customCards??[],window.customCards.push({type:"alert-redux-admin-card",name:"Alert Redux admin",description:"Lists every Alert Redux alert, and lets admins disable, enable, and suspend them."}));
